...
import { type RobinhoodAssetConfig } from '@/lib/robinhood'
 ...
 */
export function AssetCard({ asset, selected = false, onSelect, className }: AssetCardProps) {
  const handleClick = () => {
 ...
 */
export function AssetCardCompact({ asset, selected, onSelect, className }: AssetCardProps) {
  return (
    <button
      ...
      aria-pressed={selected}
    >
      <AssetIcon symbol={asset.symbol} icon={asset.icon} logoUrl={asset.logoUrl} size={32} />
