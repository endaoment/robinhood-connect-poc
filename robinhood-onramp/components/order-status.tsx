...
import type { OrderStatus, OrderStatusResponse } from '@/types/robinhood'
import { AlertCircle, CheckCircle, Clock, Copy, ExternalLink, RefreshCw } from 'lucide-react'
import { useCallback, useEffect, useState } from 'react'

/**
 * ⚠️ NOTE: This component is for OFFRAMP use only
 *
 * The Robinhood order status API does NOT work for onramp flows.
 * For onramp, all necessary data is already available in the callback URL.
 *
 * This component is kept for potential future offramp implementation.
 */
...
export function OrderStatusComponent({ referenceId, onStatusChange, autoRefresh = true }: OrderStatusProps) {
  ...
  const setupPolling = useCallback(() => {
    ...
    if (state.orderStatus.status === 'ORDER_STATUS_SUCCEEDED' || state.orderStatus.status === 'ORDER_STATUS_FAILED') {
      return
    }
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        {/* Last Updated */}
        {state.lastUpdated && (
          <div className="text-xs text-zinc-500">Last updated: {state.lastUpdated.toLocaleTimeString()}</div>
        )}
