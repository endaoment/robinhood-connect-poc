 ...
 * @param referenceId - UUID v4 generated during onramp initiation
...
export async function redeemDepositAddress(referenceId: string): Promise<DepositAddressResponse> {
  ...
  try {
    ...
    if (!isAssetSupported(responseData.assetCode)) {
      ...
      throw new RobinhoodAPIError(`Asset ${responseData.assetCode} is not supported for transfers`, 'UNSUPPORTED_ASSET')
    }
 ...
 * @param referenceId - UUID v4 from onramp order
